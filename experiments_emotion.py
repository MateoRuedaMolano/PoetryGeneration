...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="The sentiment for generation: positive, negative, impartial, mixed"
)
...
logging.basicConfig(
    ...
    filename=("../log/positive/mcts_{}_{}_{}_{}_testgit.log".format(args.c, args.temperature, args.penalty, args.num_it)) 
)
logger = logging.getLogger(__name__)

###################### DISCRIMINATOR
tokenizerBERT = AutoTokenizer.from_pretrained("nickwong64/bert-base-uncased-poems-sentiment")
classifierModel = AutoModelForSequenceClassification.from_pretrained("nickwong64/bert-base-uncased-poems-sentiment")
classifierModel.eval()
classifierModel.to("cuda")
...
def get_values(tokens_ids, label):
#    """Gets sequence scores from the discriminator"""
    ...
    inputs = tokenizerBERT(propositions[0], return_tensors="pt")
    inputs.to("cuda")
    with torch.no_grad():
        logits = classifierModel(**inputs).logits
    
    outputs = logits[0][label]
    outputs = outputs.cpu().numpy()
    
    print(propositions[0], outputs)
    return outputs
...
def root_fun(original_input, temperature, repetition_penalty, labelSentiment):
    ...
    values = get_values(original_input.input_ids, labelSentiment)
   

    return priors, values, states

#Delete labels as a parameter
def rec_fun(states, token_ids, attention_masks, temperature, repetition_penalty, labelSentiment):
    ...
    with torch.no_grad():
        ...
        #ROLLOUT_SIZE
        while(not is_finished.all() and len(token_ids[0]) < 72):
            with torch.no_grad():
                outputs = gpt(
                    **model_inputs,
                    return_dict=True,
    ...
    # Use of our discriminator to get values
    #Delete labels as a parameter, root=False
    values = get_values(token_ids, labelSentiment)
...
class BatchedMCTS():
    def __init__(self, root_fun, rec_fun, batch_size, num_simulations, num_actions, num_sparse_actions, pb_c_init, temperature, alpha, penalty, labelSentiment):
        ...
        self._labelSentiment = labelSentiment
        self.alpha = alpha
    ...
    def search(self, original_input):
        ...
        # Evaluate the root.
        prior, values, states = self._root_fun(original_input, self._temperature, self._repetition_penalty, self._labelSentiment)
    ...
    def expand(self, node_indices, actions, next_node_index):
        ...
        # Evaluate nodes.
        (prior, values, next_states) = self._rec_fun(states, tokens_ids, attention_masks, self._temperature, self._repetition_penalty, self._labelSentiment)
...
def main():
    ...
    if sentiment == 'n':
        ...
        raise ValueError("Sentiment needs to be p (positive), n (negative), i (impartial) or m (mixed)")
        
    
        
    prompt_texts = [None] * batch_size
    MCTS = BatchedMCTS(root_fun, rec_fun, batch_size=batch_size, num_simulations=args.num_it, num_actions=vocab_size+1, num_sparse_actions=50, pb_c_init=args.c, temperature = args.temperature, alpha=args.alpha, penalty=args.penalty, labelSentiment = labelSentiment)
    ...
    while(generated_counter + batch_size <= samples_size): 
        ...
        for i in range(0,len(batchSonnets)):
            prompt_texts[i] = "<|startoftext|> " + str(batchSonnets[i].splitlines()[0]) + ""
        ...
        tokens_pbar = tqdm(total = 60, desc="Tokens generated") # 23
        for i in range(0, 56):
            #pdb.set_trace()
